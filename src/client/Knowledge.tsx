...
import { SynthesisClaims } from './SynthesisClaims.tsx'
import { KnowledgeSynthesis } from './KnowledgeSynthesis.tsx'
...
interface KnowledgeContextValue {
  ...
  readonly open: (cardId: string, readRevisionId?: string) => void
  readonly synthesize?: (topicId: string) => void
...
interface KnowledgeDialog {
  readonly id: string
  readonly content: { readonly cardId?: string; readonly readRevisionId?: string; readonly synthesisTopicId?: string; readonly editRevisionId?: string; readonly source?: KnowledgeDiscussionAddress; readonly extraction?: KnowledgeDiscussionAddress }
...
export function KnowledgeProvider({ api, topics, read, t, children, useSessions }: {
  ...
  return <KnowledgeContext.Provider value={{ api, refresh, changed: () => { setRefresh(value => value + 1) }, topicId, selectTopic, saved, dismissSaved: () => { setSaved(undefined) },
    create: source => { open(source === undefined ? {} : { source }) }, open: (cardId, readRevisionId) => { open({ cardId, ...(readRevisionId === undefined ? {} : { readRevisionId }) }) },
    ...(api.prepareSynthesis === undefined || api.synthesize === undefined || useSessions === undefined ? {} : { synthesize: (synthesisTopicId: string) => { open({ synthesisTopicId }) } }),
    ...
    {dialogs.map(({ id, content: dialog }, index) => {
      ...
      const title = dialog.synthesisTopicId !== undefined ? 'synthesis.title' : dialog.extraction !== undefined ? 'extract.title' : 'knowledge.title'
      ...
      {dialog.synthesisTopicId !== undefined && useSessions !== undefined ? <KnowledgeSynthesis topicId={dialog.synthesisTopicId} api={api} topics={topics} read={read} useSessions={useSessions} t={t}
        changed={() => { setRefresh(value => value + 1) }} /> : dialog.extraction === undefined ? <KnowledgeEditor cardId={dialog.cardId} readRevisionId={dialog.readRevisionId} editRevisionId={dialog.editRevisionId} source={dialog.source} topicId={topicId}
...
export function KnowledgeEditor({ cardId, readRevisionId, synthesisDraft, frozenMaterials, editRevisionId, source, topicId, api, topics, read, close, changed, onSaved, draft, preparation, scrollContainer, t }: {
  readonly readRevisionId?: string | undefined
  readonly synthesisDraft?: SynthesisDraft
  readonly frozenMaterials?: readonly ResearchMaterial[]
  ...
  const [identity] = useState(() => cardId ?? synthesisDraft?.cardId ?? draft?.cardId ?? crypto.randomUUID())
  const [card, setCard] = useState<KnowledgeCard>()
  const directEditInitialized = useRef(false)
  const [version, setVersion] = useState(readRevisionId ?? '')
  ...
  const [content, setContent] = useState<KnowledgeContent>(synthesisDraft?.content ?? draft?.content ?? EMPTY_CONTENT)
  const [synthesis, setSynthesis] = useState<SynthesisSave | undefined>(synthesisDraft?.synthesis)
  const [synthesisMaterials, setSynthesisMaterials] = useState<readonly ResearchMaterial[]>(frozenMaterials ?? [])
  const invalidEditedCitations = synthesis !== undefined && verifySynthesisClaims(synthesisMaterials, synthesis.claims).invalidCitations > 0
  const [sources, setSources] = useState<readonly KnowledgeSourceAddress[]>(draft?.sources ?? (source === undefined ? [] : [source]))
  const [invalidCitations, setInvalidCitations] = useState(synthesisDraft?.invalidCitations ?? draft?.invalidCitations ?? 0)
  ...
  useEffect(() => {
    ...
    if (cardId !== undefined) {
      ...
      void api.read({ cardId }, controller.signal).then(value => {
        ...
        const addressed = editRevisionId ?? readRevisionId
        const revision = addressed === undefined ? value.revisions.at(-1)! : value.revisions.find(item => item.revisionId === addressed)
  ...
  }, [api, topics, cardId, editRevisionId, readRevisionId, reload])
  useEffect(() => {
  ...
  return <div ref={editorRef} className={`${styles.knowledgeBody} ${styles.knowledgeEditor}`} data-working-scroll={scrollContainer === undefined ? '' : undefined}>
    ...
    {loading || (cardId !== undefined && card === undefined) ? null : <>
      ...
      {editing ? <form className={styles.knowledgeForm} onSubmit={event => {
        ...
        if (busy || prefilling || invalidEditedCitations) return
        ...
        {(draft !== undefined && sources.length === 0 && synthesis === undefined) ? <p role="status">{t('extract.verify')}</p> : null}
        ...
        {(['title', 'conclusion'] as const).filter(field => synthesis === undefined || field === 'title').map(field => <label key={field}>{t(`knowledge.field.${field}`)}
        ...
        {synthesis === undefined ? null : <><p>{t('synthesis.manual')}</p><SynthesisClaims claims={synthesis.claims} materials={synthesisMaterials}
          busy={busy} read={read} t={t} onChange={claims => { setSynthesis({ ...synthesis, claims }); setInvalidCitations(0) }} /></>}
        <details className={styles.optionalFields}><summary>{t('knowledge.moreFields')}</summary>
          <label>{t('knowledge.topic')}<select value={selectedTopic} disabled={busy || synthesis?.source.kind === 'preparation'} onChange={event => { setSelectedTopic(event.target.value) }}>
            <option value="">{t('knowledge.noTopic')}</option>
            {topicList.map(topic => <option key={topic.topicId} value={topic.topicId}>{topic.title}</option>)}
          </select></label>
          {(['question', 'rationale', 'openQuestions'] as const).filter(field => synthesis === undefined || field === 'question').map(field => <label key={field}>{t(`knowledge.field.${field}`)}
        ...
        {synthesis === undefined ? <p>{prefillInfo ? t('workbench.sourceTurns', { first: prefillInfo.first, last: prefillInfo.last }) : t('knowledge.sourceHint')}</p> : null}
        ...
        <details hidden={synthesis !== undefined}><summary>{t('knowledge.sourceDetails')}</summary>
        ...
        <div className={styles.editorActions}><span role="status">{t(busy ? 'topic.saving' : dirty ? 'knowledge.unsaved' : card === undefined ? 'knowledge.draftHint' : 'knowledge.noChanges')}</span><button className={styles.primaryButton} type="submit" disabled={busy || prefilling || invalidEditedCitations || content.title.trim() === ''}>{t('knowledge.save')}</button>
