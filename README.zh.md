...
进入 **研图** 后，在 **图谱 / 阅读** 之间切换。图谱用于整理关系与发散思路；阅读视图提供知识目录、完整正文、准确来源和后续研究。**研究范围 → 跨工作区主题** 可把同一 Host 的资料放在一起，阅读视图也可查看此 Host 的全部知识。

在主题中切换图谱与阅读，会保留选中的知识卡片、显示修订、展开来源、阅读滚动位置和已有画布位置。保存状态与“待核实 / 你已确认”分开展示。卡片的**继续讨论**直接带入当前展示的修订，**编辑卡片**直接编辑这一版本并另存新修订。阅读旧版本时，导出按钮明确标为“导出最新版本”；导出预览会列出实际版本。
...
首轮正式实现的范围与证据见[工作台验收记录](docs/reviews/research-workbench-acceptance.md)。

## 知识卡片

在 **研图 → 原文** 中选择已完成轮次，点击**保存为知识卡片**。原文会预填可编辑的标题、问题和结论；载入较慢时不会覆盖你已输入或主动清空的字段。默认只展示标题和结论，其余内容按需展开，来源以讨论标题和轮次展示。超过 24,000 字符的初稿会明确标为节选，完整所选来源仍随保存保留。点击**保存知识**创建卡片，可选择所属主题；保存不等于事实已核实。人工创建不调用模型、不修改源会话。
...
从首页进入**阅读**或**更多 → 知识卡片**，也可在**搜索讨论与知识**中切换内容按钮，按标题或正文检索当前 Host 全部知识或所选主题。卡片归属不由来源目录推断。移出主题保留内容、修订和来源，可搜索后重新加入。卡片保存在 Host，清理浏览器缓存或重启 Host 后仍可恢复；重置与重新布局仅改变展示。每次保存最多 32 个来源、4 MB 来源文本 JSON，超限时需缩小选区。
