...
export function apply(ctx) {
  ...
  if (!root) throw new Error('Run this fixture with pnpm prototype:dsh')
  const provider = 'research-prototype'
  const model = 'demo'
  const answers = [
    '缓存命中率是一个线索，端到端延迟才是用户实际感受到的结果。即使命中率提高，序列化、网络传输和失效维护也可能抵消收益。\n\n可以先记录基线，再分别观察命中与未命中请求的 P95 延迟。把缓存维护开销单独列出，才能知道优化发生在哪里。\n\n适用条件：读多写少、重复访问较多，且允许短暂的数据延迟。对强一致性的敏感数据，需要单独设计读取与失效策略。',
