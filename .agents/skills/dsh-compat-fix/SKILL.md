...
2. 确认 Harness checkout 已构建（`build:native-system`、`build:lib`，冒烟还需 `build:web`），然后 `DSH_HARNESS_ROOT=<checkout> pnpm check:harness`（四个编译面 + 完整 Harness 套件）
3. `pnpm pack --pack-destination .artifacts` 后 `DSH_HARNESS_ROOT=... pnpm smoke:harness`（实包隔离 profile：安装/离线恢复/研究读写/移除，固定模型 19 次）
4. `pnpm preview:dsh` 起隔离预览，真实浏览器打开样例讨论切到「研图」，核对徽标 `Research Graph v<版本> · <Build ID>` 与图谱渲染；用完 `pnpm preview:dsh --stop`

## 4. PR 与合并

- 分支 `feat/dsh-<目标版本>`（或 `fix/...`），Conventional Commit；PR 正文写清用户可见影响、上游破坏性变化清单、验证证据。排除未跟踪研究资料。
- 盯 CI 四项（Node 22.19/24/26 + Matching DSH release）；失败先拉日志定位，修好再推。
...
4. 正式包回验（与候选分开，不混用哈希）：`registry.npmjs.org` 的 integrity/shasum 对上下载字节，SLSA provenance 核对仓库/tag/publish.yml/运行 ID 与归档摘要，官方字节再跑一次隔离冒烟，Release 挂归档 + SHA256SUMS 并下载回读，验收记录补正式数值。
