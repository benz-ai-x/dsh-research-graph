...
Current plugin **0.1.7-rc.2** uses npm tag `next`; the pinned command avoids installing an incompatible older package. See [Install and upgrade](#install) for local archives and migration from the previous package name.
...
|---|---|---|---|
| `@benz-ai-x/dsh-research-graph@0.1.7-rc.2` | `0.1.7-rc.2` | `^22.19.0 \|\| >=24.0.0` | Real Host/Client types, integration tests and packed-profile runtime acceptance |

Current prerelease **0.1.7-rc.2** targets **DSH 0.1.7-rc.2**: the first adaptation of this DSH line. No upstream breaking change reaches the plugin's surfaces; the one neighboring behavior change is Host-side — Session model selection now requires catalog membership, which only the bundled test fixtures follow by advertising their fixed model. This release also ships the Session projection facts merged earlier for this line: entering the graph reads non-activating Session projections, cards show the closed-turn count and a hover preview of the newest prompt, the inspector gains a Session statistics block, and Branch discussions never opened natively recover their durable titles in the graph. See the [release acceptance record](reviews/release-0.1.7-rc.2.md) for official-package verification and known limitations.
...
The [DSH plugin manager](https://github.com/deepseek-ai/deepseek-harness/blob/dsh-v0.1.7-rc.2/apps/cli/reference/README.md#plugin-management) invokes `pnpm`, which must be on your `PATH`. Use the launcher for your matching DSH installation throughout the commands below:

- Installed CLI: use `dsh`.
- npm launcher: replace `dsh` with `npx @deepseek-ai/dsh@0.1.7-rc.2`.
- Source checkout: build the `dsh-v0.1.7-rc.2` checkout using the [upstream setup](https://github.com/deepseek-ai/deepseek-harness/blob/dsh-v0.1.7-rc.2/README.md#run-from-source), run from that checkout, and replace `dsh` with `pnpm dsh`.
...
Session, LLM, and browser runtime services remain owned by the selected dsh profile. The plugin declares its Typert protocol dependency explicitly, with the matching LLM as a peer dependency. The offline recovery command bundles its format catalog and libraries so it also works before the Host starts. All directly referenced `@deepseek-ai/dsh-*` packages are pinned to the target DSH version (`0.1.7-rc.2` for plugin `0.1.7-rc.2`).
...
DSH `0.1.7-rc.2` disables full-text indexing by default. If Research Graph reports **Full-text indexing is not enabled**, add this override to the active profile's `cordis.patch.yml` (for the web profile, `$DSH_HOME/profiles/web/cordis.patch.yml`):
...
- Arbitrarily dense graphs or physically overlapping cards may still have crossings or obstructed terminals. The [release acceptance](reviews/release-0.1.7-rc.2.md) describes the verified geometry scenarios.
