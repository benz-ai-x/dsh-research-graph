...
日期：2026-09-12。需求与 TODO：[Issue #29](https://github.com/benz-ai-x/dsh-research-graph/issues/29)。基线为摘要修复 PR #28 的 `a46c1de`，本轮 PR 依赖 #28；尚未合并、发布或替换用户的日常 profile。
...
本轮体验构建仍使用 `0.1.5-rc.2.1`，Build ID **local-af881cf6**，不是 npm 上的新发布。最终验收归档 422561 bytes，SHA-256 `a18983cea35a07e32ac038d968ea3acbb54e213320db54488187ef87240545fb`。日志和隔离 profile 在工作树私有 `.artifacts/reading-ux/` 与 `.artifacts/workbench-dsh/`；令牌不进入 Git。
...
本轮优化阅读与捕获闭环。主题管理栏、目录中的纯文本节选和更丰富的节点预览仍可继续细化；知识语义关系、自动研究建议、句级来源、跨 Host 汇聚及跨重启草稿恢复未纳入。最终 UI/UX 评价由用户人工体验决定。用户原有 Host、会话、配置和其他预览服务保持原样。
