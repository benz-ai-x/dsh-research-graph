...
The prototype keeps the selection and an already-created retry target across its three layouts. After successful capture, it adds both sources and the target to the selected Research Topic so the many-to-one relationship remains visible. If topic association fails, retry reuses the captured target. This does not introduce cross-Host research or change permissions.
