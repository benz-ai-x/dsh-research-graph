...
- 进入图谱时会为每个讨论读取免激活的会话投影：卡片元信息行增加已完成轮数，悬停卡片可见最近一轮提问预览；检查器增加**会话统计**区块，展示模型、轮/步数与耗时、累计 tokens、上下文占用文本、目标与待办进度及最近一轮问答预览。这些信息是 Host 保留事实的呈现，不激活会话、不调用模型；读取失败时卡片保持原样。上下文占用只是不同采样时刻的参考读数，不作为配额或告警依据。未原生打开过的分支讨论也会借此恢复持久标题，不再停留在目录名。
